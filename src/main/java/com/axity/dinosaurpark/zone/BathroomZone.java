...
public class BathroomZone implements ParkZone {
    ...
    private final String name = "Baños";
    ...
    @Override
    public boolean hasCapacity() { return occupiedSlots.size() < maxCapacity; }
    
    @Override
    public int getCurrentOccupancy() { return occupiedSlots.size(); }
    ...
    public void tick() {
        ...
        for (Tourist t : toExit) {
            exit(t);
        }
    }
    
    public boolean tryEnter(Tourist tourist, Random rng) {
        if (tourist == null || !hasCapacity()) return false;
        enter(tourist);
